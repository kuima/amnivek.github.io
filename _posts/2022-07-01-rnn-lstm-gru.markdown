...
1. 首先，前一时刻的隐藏状态 $$ \boldsymbol{h}_{t-1} $$ 和当前时刻的输入 $$ \boldsymbol{x}_t $$ 一起传入四个全连接层，其中三个使用 $$ sigmoid $$ 作为激活函数生成三个不同的门控向量（遗忘门 $$ \boldsymbol{f}_t $$，输入门 $$ \boldsymbol{i}_t $$，输出门 $$ \boldsymbol{o}_t $$），剩余一个使用 $$ \tanh $$ 作为激活函数生成输入门的输入数据（$$ \boldsymbol{g}_t $$）；
...
3. 增加一个称为重置门（reset gate）的门控单元 $$ \boldsymbol{r}_t $$，用来控制前一时刻状态的哪些部分可以被添加到记忆中。
